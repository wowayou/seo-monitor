...
*   **反爬虫**：脚本已内置 `User-Agent` 伪装和常见追踪代码（GA, Facebook Pixel）屏蔽，以提升速度并降低被拦截概率。
...
*   [ ] **URL 列表增强**: 补充产品详情页与新闻详情页的采样，微调页面对应关系。
*   [ ] **图像对比**: 集成 SSIM 图像识别算法，自动对比今日截图与基准图，实现像素级异常报警。

---

## ⚠️ 已知局限 (Known Limitations)

自动化巡检能覆盖绝大多数日常场景，但以下情况仍需人工介入。这些是实际使用中总结出的边界：

1.  **Cloudflare 假验证 / 延迟触发的验证组件**
    部分站点会在页面停留数秒后注入伪造的 Cloudflare 人机验证层。自动化脚本的浏览器指纹与交互模式和真人不同，滚动路径机械、速度过快，可能不会触发这类验证 —— 反而"访问成功"。脚本已加入随机停留、鼠标移动、页面滚动的真人行为模拟，并检测异常全屏 iframe，但无法保证覆盖所有变体。

2.  **301 / 短链跳转难以察觉**
    某些页面进入后会跳转到外部短链或其它域名。此时状态码与截图往往都"正常"，脚本不易判定为异常，但实际已影响用户体验。这类问题目前仍依赖人工复核。

3.  **截图版式偏差**
    使用了 `sticky-header`（吸顶导航）的站点，整页截图时页眉可能出现在非预期位置。属于渲染时机问题，一般不影响状态判断，但视觉报告需留意。

4.  **自动化无法完全替代人工**
    以上情况说明：脚本的价值在于把重复性巡检的工作量降到最低，快速筛出明显异常；对于验证层、跳转、体验类的细节，建议保留一份人工抽检清单作为补充。**关键页面仍建议打开截图确认。**
